...
require('./referentiels.js')(router);

require('./activities.js')(router);
require('./competences.js')(router);
require('./levels.js')(router);
...
router.post('/asks/confirmations', (req, res, next) => {
  ...
  User.findOneAndUpdate(query, req.newData, { upsert: true }, function(err, doc) {
    ...
    AskRoleConfirmation.findOneAndUpdate({ _id: req.body.ask_id }, { actif: true }, (err, doc) => {
      if (err) return res.send(500, { error: err });
    });

    return res.redirect('/admin/asks/confirmation');
  });
});
